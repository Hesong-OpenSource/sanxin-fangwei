...
//		}
	...
	*/	
	...
	function modify_advance($data,$user_id,$log_msg='',$param=array())
	{
		...
		$money=doubleval($data['money']);
 		$mortgage_money=doubleval($data['mortgage_money']);
 		 if(($user['mortgage_money']+$mortgage_money)>=0&&($user['money']+$money)>=0){
		 	if(floatval($data['mortgage_money'])!=0)
			{
				$sql = "update ".DB_PREFIX."user set mortgage_money = mortgage_money + ".doubleval($data['mortgage_money'])." where id =".$user_id;
				$GLOBALS['db']->query($sql);
			}
			if(floatval($data['money'])!=0){
			...
			if(doubleval($data['ben_money'])!=0
			||doubleval($data['earn_money'])!=0
			||doubleval($data['buy_money'])!=0
			||doubleval($data['fee'])!=0 
			||doubleval($data['buy_frozen'])!=0
			||doubleval($data['service_fee'])!=0
			||doubleval($data['pay_money'])!=0
			||doubleval($data['organiser_fee'])!=0
			){
				
				licai_log($data,$user_id,$log_msg);
 			}
			elseif(doubleval($data['money'])!=0)
			{
				$log_info['log_info'] = $log_msg;
				$log_info['log_time'] = get_gmtime();
				$adm_session = es_session::get(md5(app_conf("AUTH_KEY")));
				...
				if($data['mortgage_money']){
					$log_info['money'] = doubleval($log_info['money']+$data['mortgage_money']);
				}
	...
	function licai_log($data,$user_id,$log_msg){
				...
				$log_info['user_id'] = $user_id;
 				if( doubleval($data['ben_money'])!=0){
					...
					$log_info['type'] = 8;
					$log_info['money'] = doubleval($data['ben_money']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
  				if(doubleval($data['earn_money'])!=0){
					//赎回收益
	 				$log_info['log_info'] = "赎回收益";
					$log_info['type'] = 9;
					$log_info['money'] = doubleval($data['earn_money']);
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				if(doubleval($data['fee'])!=0){
					//赎回手续费
	 				$log_info['log_info'] = "赎回手续费";
					$log_info['type'] = 10;
					$log_info['money'] = doubleval($data['fee']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				
				if(doubleval($data['organiser_fee'])!=0){
					//赎回手续费
	 				$log_info['log_info'] = "平台收益"; 
					$log_info['type'] = 10;
					$log_info['money'] = doubleval($data['organiser_fee']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				
				if( doubleval($data['buy_money'])!=0){
					//赎回本金
	 				$log_info['log_info'] = "理财购买本金";
					$log_info['type'] = 11;
					$log_info['money'] = doubleval($data['buy_money']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				if( doubleval($data['buy_fee'])!=0){
					//赎回本金
	 				$log_info['log_info'] = "理财购买手续费";
					$log_info['type'] = 12;
					$log_info['money'] = doubleval($data['buy_fee']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				if( doubleval($data['buy_frozen'])!=0){
					//赎回本金
	 				$log_info['log_info'] = "理财冻结资金";
					$log_info['type'] = 13;
					$log_info['money'] = doubleval($data['buy_frozen']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				
				if( doubleval($data['service_fee'])!=0){
					//赎回本金
	 				$log_info['log_info'] = "理财服务费";
					$log_info['type'] = 14;
					$log_info['money'] = doubleval($data['service_fee']);
	 				
					$GLOBALS['db']->autoExecute(DB_PREFIX."user_log",$log_info);
				}
				
				if( doubleval($data['pay_money'])!=0){
					//赎回本金
	 				$log_info['log_info'] = "理财发放资金";
					$log_info['type'] = 15;
					$log_info['money'] = doubleval($data['pay_money']);
